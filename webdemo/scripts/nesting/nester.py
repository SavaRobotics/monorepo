#!/usr/bin/env python3
"""DXF nesting tool for Mastra - combines downloading and nesting functionality."""
...
class DXFNester:
    ...
    def _extract_collision_polygon(self, doc, msp):
        ...
        try:
            ...
            try:
                safety_buffer = 2.0  # Default buffer
    ...
    def generate_nested_dxf(self, placed_items):
        ...
        current_dir = Path.cwd() / 'webdemo' / 'src' / 'mastra' / 'tools' / 'nesting'
        temp_dir = current_dir / 'temp'
        temp_dir.mkdir(parents=True, exist_ok=True)
        
        output_path = temp_dir / 'nested_layout.dxf'
...
async def nest_dxf_parts(
    ...
    """
    Main function to download DXF files and perform nesting.
    
    Args:
        dxf_urls: List of URLs to DXF files
    ...
    Returns:
        Dictionary with nesting results including output file path
    """
    try:
        # Get the directory for temp files (in the tools/nesting directory)
        current_dir = Path.cwd() / 'webdemo' / 'src' / 'mastra' / 'tools' / 'nesting'
        temp_dir = current_dir / 'temp'
        temp_dir.mkdir(parents=True, exist_ok=True)
        ...
        response = {
            ...
            "total_parts": len(dxf_urls),
            "unfittable_count": len(result.get('unfittable_parts', [])),
...
if __name__ == "__main__":
    ...
    result = asyncio.run(nest_dxf_parts(test_urls))
